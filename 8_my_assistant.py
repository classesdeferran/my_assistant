...
from KEY_Weather import KEY_WEATHER
...
def meteo(ciutat):
    ...
    humitat = data['main']['humidity']
    cel = data['weather'][0]['description']

    return temp_actual, temp_minima, temp_maxima, temp_sensacion, humitat, cel
...
def funcion_ppal():
    ...
    while True:
        ...
        elif "cuál es el tiempo en" in peticion:
            respuesta_PC("Ahora te lo digo")
            peticion = peticion.replace("cuál es el tiempo en", "")
            temp_actual, temp_minima, temp_maxima, temp_sensacion, humitat, cel = meteo(peticion)
            respuesta_PC(f"La temperatura actual en {peticion} es de {round(temp_actual)} grados, con sensación de {round(temp_sensacion)}")
            respuesta_PC(f"{cel} y humedad del {humitat} por ciento")
            respuesta_PC(f"Temperatura mínima de {round(temp_minima)} y maxima de {round(temp_maxima)}")
            continue
        ...
        elif ("programa finalizado" in peticion) or ("eso es todo" in peticion):
